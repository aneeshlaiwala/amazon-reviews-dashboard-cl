...
def advanced_sentiment_analysis_multilevel(text, model_choice="DistilBERT (English)", openai_api_key=None,
                                           google_api_key=None, fallback_info=None):
...
for i, (topic, count) in enumerate(top_topics.items(), 1):
    ...
    formatted_topics.append(
        f"**{i}. {topic}**: {count:,} mentions ({count / total_reviews * 100:.1f}%) - {impact_level} Impact")
...
sentiment_score = (sentiment_dist.get('Extremely Positive', 0) + sentiment_dist.get('Very Positive',
                                                                                    0) + sentiment_dist.get('Positive',
                                                                                                            0)) * 0.4  # 40% weight
...
for i, text in enumerate(df['reviewText']):
    result = advanced_sentiment_analysis_multilevel(text, model_choice=model_choice, openai_api_key=openai_api_key,
                                                    google_api_key=google_api_key, fallback_info=fallback_info)
...
engagement_rate = len(detailed_reviews) / total_reviews * 100
brand_health_score = int((avg_rating / 5.0) * 40 + (
            sentiment_dist.get('Extremely Positive', 0) + sentiment_dist.get('Very Positive', 0) + sentiment_dist.get(
        'Positive', 0)) * 0.4 + (100 - total_risk_rate) * 0.2)
...
with col1:
    ...
    if sentiment_model == "Google Natural Language API":
        google_api_key = st.text_input("Enter your Google Cloud API Key (JSON path):", type="password",
                                       key="google_api_key")
...
if uploaded_file or st.session_state.processed_data is not None:
    if uploaded_file and st.session_state.processed_data is None:
        with st.spinner('🤖 Processing with AI...'):
            ...
            if df is not None:
                df, topics, fallback_info = process_data_with_advanced_ml(df, model_choice=sentiment_model,
...
st.markdown('<div class="active-filters">📌 <b>Note:</b> The insights below reflect only the filtered dataset.</div>',
            unsafe_allow_html=True)
...
st.markdown("<h3 style='text-align:center; margin-bottom:1rem;'>📈 Business Impact Score Penetration Trend</h3>",
            unsafe_allow_html=True)
...
filtered_df['impact_category'] = pd.cut(filtered_df['businessImpact'], bins=impact_bins, labels=impact_labels,
                                        include_lowest=True)
...
st.markdown("<h3 style='text-align:center; margin-bottom:1rem;'>📈 Sentiment Trend Over Time</h3>",
            unsafe_allow_html=True)
...
sentiment_order = ['Extremely Negative', 'Very Negative', 'Negative', 'Neutral', 'Positive', 'Very Positive',
                   'Extremely Positive']
...
st.markdown("<h3 style='text-align:center; margin-bottom:1rem;'>🏷️ Topic Penetration Trend</h3>",
            unsafe_allow_html=True)
...
topic_trend = filtered_df[filtered_df['topic'].isin(top_topics)].groupby(['year', 'month', 'topic']).size().reset_index(
    name='count')
...
st.markdown("<h3 style='text-align:center; margin-bottom:1rem;'>📊 Customer Rating Distribution</h3>",
            unsafe_allow_html=True)
...
with col2:
    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
st.markdown("<h3 style='text-align:center; margin-bottom:1rem;'>🛡️ Customer Sentiment Analysis</h3>",
            unsafe_allow_html=True)
...
st.markdown(
    "<div class='insight-box'><div class='insight-title'>🔑 Executive Insight</div> No significant keywords found.</div>",
    unsafe_allow_html=True)
else:
...
st.markdown("""
                ...
                **Strategic Use:** Prioritize high-impact positive reviews for marketing and address high-impact negative reviews immediately.
                """)
st.markdown('</div>', unsafe_allow_html=True)
# Place the Business Impact Score Penetration chart here:
impact_bins = [-5, -3, -1.1, 1, 3, 5]
impact_labels = [
    'Crisis-level (-5 to -3)',
    'Negative (-2.9 to -1.1)',
...
st.markdown('<div class="chart-container">', unsafe_allow_html=True)
st.markdown("<h3 style='text-align:center; margin-bottom:1rem;'>📊 Business Impact Score Penetration</h3>",
            unsafe_allow_html=True)
...
st.markdown(f"<div class='insight-box'><div class='insight-title'>🔑 Executive Summary</div> {summary}</div>",
            unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
negative_verbatims = negative_verbatims.nsmallest(5, 'businessImpact') if len(
    negative_verbatims) > 0 else pd.DataFrame()
...
st.markdown(
    f"**Champion Voice #{idx}** | ⭐{review['rating']}/5 | Business Impact: **{review['businessImpact']:.1f}/5.0**")
...
st.markdown(
    f"**Critical Issue #{idx}** | ⭐{review['rating']}/5 | Business Impact: **{review['businessImpact']:.1f}/5.0**")
...
col_a, col_b = st.columns(2)
with col_a:
    urgency = "🚨 Critical" if review['businessImpact'] < -3 else "⚠️ High" if review[
                                                                                  'businessImpact'] < -1 else "📊 Monitor"
...
total_reviews = len(filtered_df)
st.markdown(
    "<div style='font-size:0.95rem; color:#764ba2; margin-bottom:0.5rem;'><b>Note:</b> Penetration % can sum to more than 100% because reviews may be tagged with multiple topics.</div>",
    unsafe_allow_html=True)
...
risk_trend = "📈" if high_risk_count > len(filtered_df) * 0.1 else "📊" if high_risk_count > len(
    filtered_df) * 0.05 else "📉"
st.markdown(f"""
    <div class='metric-card'>
...
st.markdown(f'<div class="insight-box"><div class="insight-title">🔑 Executive Insight</div> {insight_text}</div>',
            unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)
...
positive_pct = (filtered_for_sidebar['sentiment'].str.contains('Positive', na=False)).sum() / len(
    filtered_for_sidebar) * 100
