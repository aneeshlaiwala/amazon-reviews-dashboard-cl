...
def main():
    ...
    with tab1:
        ...
        with col1:
            fig1 = px.histogram(filtered_df, x='rating', title="Star Rating Distribution", color_discrete_sequence=['#FF9500'])
            st.plotly_chart(fig1, use_container_width=True)
            
            sentiment_counts = filtered_df['sentiment'].value_counts()
            fig2 = px.pie(values=sentiment_counts.values, names=sentiment_counts.index, title="Sentiment Distribution",
        ...
        with col2:
            ...
            fig3 = px.line(trends, x='date', y='count', title="Review Volume Over Time", color_discrete_sequence=['#667eea'])
            st.plotly_chart(fig3, use_container_width=True)
            
            heatmap = pd.crosstab(filtered_df['rating'], filtered_df['sentiment'])
            fig4 = px.imshow(heatmap.values, x=heatmap.columns, y=heatmap.index, title="Rating vs Sentiment Heatmap", color_continuous_scale='Blues')
            st.plotly_chart(fig4, use_container_width=True)
    
    with tab2:
        st.markdown("## 🔍 Verbatim Analysis")
        ...
        with col1:
            ...
            if word_freq:
                ...
                fig5 = px.bar(df_words, x='Frequency', y='Word', orientation='h', title="Top 15 Keywords", color_discrete_sequence=['#FF9500'])
        ...
        with col2:
            ...
            if topics:
                ...
                st.info("Topic modeling not available")
            
            st.subheader("Review Length")
            fig6 = px.histogram(filtered_df, x='wordCount', title="Word Count Distribution", nbins=20, color_discrete_sequence=['#667eea'])
            st.plotly_chart(fig6, use_container_width=True)
        
        if 'topic' in filtered_df.columns:
            st.subheader("Topic Distribution")
            topic_counts = filtered_df['topic'].value_counts()
            fig_topics = px.pie(values=topic_counts.values, names=topic_counts.index, title="Review Topics")
    ...
    with tab3:
        ...
        with col1:
            ...
            fig7 = px.bar(sent_rating.reset_index().melt(id_vars='rating'), x='rating', y='value', color='variable', 
                         title="Sentiment by Rating (%)", color_discrete_map={'Positive': '#28a745', 'Neutral': '#ffc107', 'Negative': '#dc3545'})
            ...
            fig8 = px.line(sent_time_pct.reset_index().melt(id_vars='year'), x='year', y='value', color='variable', 
                          title="Sentiment Trends", color_discrete_map={'Positive': '#28a745', 'Neutral': '#ffc107', 'Negative': '#dc3545'})
            st.plotly_chart(fig8, use_container_width=True)
        
        with col2:
            fig9 = px.histogram(filtered_df, x='sentimentScore', title="Sentiment Scores", nbins=20, color_discrete_sequence=['#17a2b8'])
    ...
    with tab4:
        ...
        if len(fraud_df) > 0:
            ...
            if reasons:
                ...
                fig10 = px.bar(df_reasons, x='Count', y='Type', orientation='h', title="Fraud Types", color_discrete_sequence=['#dc3545'])
    ...
    with tab5:
        ...
        col1, col2 = st.columns(2)
        with col1:
            filtered_df['lengthCat'] = pd.cut(filtered_df['wordCount'], bins=[0, 20, 50, 100, 200, float('inf')],
                                            labels=['≤20', '21-50', '51-100', '101-200', '200+'])
            
            length_rating = filtered_df.groupby('lengthCat')['rating'].mean().reset_index()
            fig11 = px.bar(length_rating, x='lengthCat', y='rating', title="Rating by Length", color_discrete_sequence=['#17a2b8'])
            st.plotly_chart(fig11, use_container_width=True)
            
            numeric_cols = ['rating', 'wordCount', 'sentimentScore']
            corr_matrix = filtered_df[numeric_cols].corr()
            fig12 = px.imshow(corr_matrix, title="Correlations", color_continuous_scale='RdBu_r')
            st.plotly_chart(fig12, use_container_width=True)
        
        with col2:
            helpful_analysis = filtered_df.groupby('rating')['helpful'].mean().reset_index()
            fig13 = px.bar(helpful_analysis, x='rating', y='helpful', title="Helpfulness by Rating", color_discrete_sequence=['#28a745'])
            st.plotly_chart(fig13, use_container_width=True)
            
            sample_size = min(500, len(filtered_df))
            fig14 = px.scatter(filtered_df.sample(sample_size), x='sentimentScore', y='rating', color='sentiment', 
                              title="Sentiment vs Rating", color_discrete_map={'Positive': '#28a745', 'Neutral': '#ffc107', 'Negative': '#dc3545'})
